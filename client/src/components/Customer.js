...
class EvaluateIpfs extends Component {
    ...
    componentDidMount(){
        ...
        this.props.addressObj.forEach((el , index) => {
            ...
            this.getIpfsData(ipfsAddress);
            });
    }
    getIpfsData = async (address) => {
        const res = await fetch(`https://ipfs.io/ipfs/${address}`)
        const data = await res.json();
        console.log('data')
        console.log(data)
        console.log('-----------------------------------------------')
        this.setState((prev) => ({ipfsDetails: [...prev.ipfsDetails, data]}))
    }
    render(){
        return ( 
            this.state.ipfsDetails.map((el,i) => (
                
                <div key={i} className="modal fade" id={"exampleModal" +el.req_id} tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    componentDidMount = async () => {
        try {
            ...
            if (reqCount.length) {
                ...
                console.log(acceptedIpfsAddress)
                    ...
                    <div className="verifier-header">
                        <div className="main-header">
                            <div className="verifier">
                                ...
                                <div className="verifier-title"><h1 style={{ fontSize: '3rem' }}>GENERAL PUBLIC</h1></div>
                            </div>
                        ...
                        <div className="recent-lists">
                            ...
                            <table className="table">
                                ...
                                <tbody>
                                    ...
                                    {this.state.acceptedIpfsAddress.length && this.renderIPFS()}
                                </tbody>
                            </table>
                        </div>
